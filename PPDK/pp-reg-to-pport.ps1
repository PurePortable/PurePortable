...
while ( $i -le $m ) {
  ...
  elseif ($key) {
    if ($s -match '^"(?<val>[^"]+)"=(?<data>.+)' -or $s -match '^(?<val>@)=(?<data>.+)') {
      $val = $Matches.val.ToLower()
      $data = $Matches.data
      if ($val -eq '@') { $val='' }
      $type = 'd'
      if ($data -match '^dword:(?<dword>[a-f0-9]+)') { # REG_DWORD
        $dword = $Matches.dword
        Add-Content -Lit $cfgfile "$key$del$val$del$type$del$dword" -Enc Unicode
      }
      ...
      elseif ($data -match '^"(?<str>.*)"$') {
        ...
        $type = 's'
        Add-Content -Lit $cfgfile "$key$del$val$del$type$del$str" -Enc Unicode
      }
      else {
